...
import (
	...
	"github.com/sDaman830/Dockerly/docker"
)

var (
	rootfsPath  string = "/tmp/dockerly/rootfs"
...
func init() {
	...
	reexec.Register("setupContainerEnv", setupContainerEnv)
...
// setupContainerEnv does all the Initialisation of the namespace
func setupContainerEnv() {
	log.Printf("\n>> INITIALIZING CONTAINER NAMESPACE ENVIRONMENT <<\n")
	...
	// set hostname
	if err := syscall.Sethostname([]byte("dockerly")); err != nil {
		log.Fatalf("Error while changing the hostname inside the container: %v", err)
	}

	executeContainerPayload()
}

// executeContainerPayload runs the shell inside the namespace after the initialisation
func executeContainerPayload() {
	...
	cmd.Env = []string{"PS1=-[dockerly]- # ", "TERM=xterm"}
...
func main() {
	...
	*/
	...
	err = docker.ExtractLayer("/tmp/dockerly/rootfs/layer.tar")
	if err != nil {
	...
	//  We’re now passing the arguments, rootfsPath, memorylimit, cpulimit, to setupContainerEnv.
	cmd := reexec.Command("setupContainerEnv")
